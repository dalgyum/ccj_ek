...
/* ----------------------------------------------------
   0. Envelope Intro Overlay Open Function & BGM Auto-Play
---------------------------------------------------- */
function playBgm() {
...
function openInvitationOverlay() {
  ...
  // Automatically play BGM when entering main invitation body
  playBgm();
}
...
function initBgmPlayer() {
  ...
  // Synchronize button UI whenever audio state changes
...
function initShareActions() {
  ...
  if (kakaoBtn) {
    kakaoBtn.addEventListener('click', () => {
      ...
      const description = '2026년 11월 21일 토요일 낮 12:00 | 예닮교회 3층 예배당';
      const imageUrl = new URL('images/hero.jpg', window.location.href).href;

      if (isKakaoInitialized && window.Kakao && window.Kakao.Share) {
        ...
        navigator.share({
          ...
          if (err.name !== 'AbortError') {
            copyToClipboard(shareUrl, '청첩장 주소가 복사되었습니다. 카카오톡에 붙여넣어 공유해 보세요!');
          }
        });
      } else {
        copyToClipboard(shareUrl, '청첩장 주소가 복사되었습니다. 카카오톡에 붙여넣어 공유해 보세요!');
...
/* ----------------------------------------------------
   10. Samsung Internet Force Dark Mode Detection & Guide
---------------------------------------------------- */
function isForcedDarkMode() {
  try {
    // 1. Check Samsung Internet User-Agent & Dark Mode state
    const isSamsung = /SamsungBrowser/i.test(navigator.userAgent);
    const isDarkMode = window.matchMedia && window.matchMedia('(prefers-color-scheme: dark)').matches;
    
    if (isSamsung && isDarkMode) {
      return true;
    }

    // 2. Fallback: Measure actual computed color from trap element
    const testBox = document.createElement('div');
    testBox.style.cssText = 'position:fixed; left:-9999px; top:-9999px; width:10px; height:10px; background-color: rgb(255, 255, 255) !important; color: rgb(0, 0, 0) !important; z-index:-1;';
    document.body.appendChild(testBox);

    const computedBg = window.getComputedStyle(testBox).backgroundColor;
    document.body.removeChild(testBox);

    if (computedBg && computedBg !== 'rgb(255, 255, 255)' && computedBg !== 'rgba(0, 0, 0, 0)') {
    ...
    console.warn('Force dark mode check error', e);
  }
  return false;
}

function openSamsungDarkModal() {
  const modal = document.getElementById('samsungDarkModal');
  if (modal) {
    modal.classList.add('active');
  }
}

function closeSamsungDarkModal() {
  const modal = document.getElementById('samsungDarkModal');
  if (modal) {
    modal.classList.remove('active');
  }
}
window.closeSamsungDarkModal = closeSamsungDarkModal;

function initForceDarkDetection() {
  // Check immediately and trigger modal
  setTimeout(() => {
    if (isForcedDarkMode()) {
      openSamsungDarkModal();
    }
  }, 400);
}
